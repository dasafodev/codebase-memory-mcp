...
static CBMPrimaryCalleeSelection select_primary_callee(CBMExtractCtx *ctx, TSNode node,
    ...
    /* Wrapper calls using same-file Dart endpoint constants need the original
     * receiver/verb even when the generic callee extractor has no literal. */
    if (!selection.name && ctx->language == CBM_LANG_DART &&
        strcmp(ts_node_type(node), "selector") == 0) {
        selection.name = dart_wrapper_http_callee(ctx, node);
    }
