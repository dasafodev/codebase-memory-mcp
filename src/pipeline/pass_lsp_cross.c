...
static int pxc_build_dart_import_map(const cbm_gbuf_t *gbuf, const char *project_name,
...
int cbm_pxc_build_import_map_with_cache(const cbm_gbuf_t *gbuf, const char *project_name,
                             const char *rel_path, CBMLanguage lang, const CBMFileResult *result,
                             CBMFileResult *const *result_cache,
                             const cbm_file_info_t *files, int file_count,
                             const char ***out_keys, const char ***out_vals, int *out_count) {
    if (lang == CBM_LANG_DART) {
        return pxc_build_dart_import_map(gbuf, project_name, rel_path, lang, result_cache,
                                         files, file_count, out_keys, out_vals, out_count);
    }
    return cbm_pxc_build_import_map(gbuf, project_name, rel_path, lang, result,
...
void cbm_pxc_run_one(CBMLanguage lang, CBMFileResult *r, const char *source, int source_len,
    ...
    cbm_pxc_run_one_with_path(lang, r, source, source_len, module_qn, NULL, defs, def_count,
                               imp_names, imp_qns, imp_count);
}

void cbm_pxc_run_one_with_path(CBMLanguage lang, CBMFileResult *r, const char *source, int source_len,
...
void cbm_pxc_dispatch_file(CBMLanguage lang, CBMFileResult *result, const char *source,
    ...
    if (lang == CBM_LANG_RUST) {
        ...
        if (shared) {
            ...
            cbm_pxc_run_one_with_path(lang, result, source, source_len, def_module, rel, file_defs,
...
int cbm_pipeline_pass_lsp_cross(cbm_pipeline_ctx_t *ctx, const cbm_file_info_t *files,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        const char **imp_vals = NULL;
        int imp_count = 0;
        cbm_pxc_build_import_map_with_cache(
            ctx->gbuf, ctx->project_name, files[i].rel_path, lang, cache[i], cache,
