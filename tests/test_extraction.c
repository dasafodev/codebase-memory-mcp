...
TEST(nest_controller_method_route_composition) {
    const char *source =
        "@Controller('bookings')\nexport class BookingsController {\n"
        "  @Get(':id')\n  findOne() {}\n"
        "}\n";
    CBMFileResult *r = cbm_extract_file(source, (int)strlen(source), CBM_LANG_TYPESCRIPT,
                                        "test", "bookings.controller.ts", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    bool found = false;
    for (int i = 0; i < r->defs.count; i++) {
        const CBMDefinition *def = &r->defs.items[i];
        if (def->name && strcmp(def->name, "findOne") == 0 && def->route_path &&
            strcmp(def->route_path, "/bookings/:id") == 0 && def->route_method &&
            strcmp(def->route_method, "GET") == 0) {
            found = true;
        }
    }
    cbm_free_result(r);
...
SUITE(extraction) {
    RUN_TEST(nest_controller_method_route_composition);
