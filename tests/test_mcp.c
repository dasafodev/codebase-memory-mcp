...
TEST(tool_cross_repo_finds_source_by_stored_root) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/cbm-mcp-cross-name-XXXXXX", cbm_tmpdir());
    ASSERT_NOT_NULL(cbm_mkdtemp(cache));
    ...
    /* All three databases share this root: the explicit target must not
     * displace the custom source, which still wins over the derived decoy. */
    char *derived = cbm_project_name_from_path(cache);
    ASSERT_NOT_NULL(derived);
    ASSERT_TRUE(mcp_cross_repo_create_project_store(cache, derived, cache));
    ...
    cleanup_project_db(cache, source_project);
    cleanup_project_db(cache, target_project);
    cleanup_project_db(cache, derived);
    free(derived);
    restore_cache_dir(saved_cache_copy);
    free(saved_cache_copy);
    cbm_rmdir(cache);
...
SUITE(mcp_mutation_guard) {
    ...
    RUN_TEST(tool_cross_repo_finds_source_by_stored_root);
