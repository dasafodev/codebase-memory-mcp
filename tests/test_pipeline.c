...
TEST(pipeline_dart_surface_preserves_callable_flags) {
    CBMFileResult result = {0};
    CBMFileResult *cache[] = {&result};
    cbm_file_info_t files[] = {{.rel_path = "lib/service.dart", .language = CBM_LANG_DART}};
    CBMLSPDef defs[] = {{.qualified_name = "fixture.lib.service.Worker.run",
                         .short_name = "run", .label = "Method", .lang = CBM_LANG_DART,
                         .callable_flags = CBM_DEF_CALLABLE_KNOWN | CBM_DEF_CALLABLE_STATIC}};
    int starts[] = {0, 1};
    cbm_lsp_surface_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(cbm_lsp_surface_build_rows(NULL, "fixture", cache, files, 1, defs, starts,
                                         &rows, &count), 0);
    ASSERT_EQ(count, 1);
    CBMArena arena;
    cbm_arena_init(&arena);
    CBMLSPDef *decoded = NULL;
    ASSERT_EQ(cbm_lsp_surface_defs_from_json(&arena, rows[0].defs_json, &decoded), 1);
    ASSERT_EQ(decoded[0].callable_flags, defs[0].callable_flags);
    char *static_sha = strdup(rows[0].surface_sha);
    cbm_store_free_lsp_surfaces(rows, count);
    defs[0].callable_flags = CBM_DEF_CALLABLE_KNOWN;
    ASSERT_EQ(cbm_lsp_surface_build_rows(NULL, "fixture", cache, files, 1, defs, starts,
                                         &rows, &count), 0);
    ASSERT_TRUE(strcmp(static_sha, rows[0].surface_sha) != 0);
    free(static_sha);
    cbm_store_free_lsp_surfaces(rows, count);
    ASSERT_EQ(cbm_lsp_surface_defs_from_json(&arena, "{\"v\":1,\"lsp\":[],\"reg\":[]}",
                                             &decoded), -1);
...
SUITE(pipeline) {
    ...
    RUN_TEST(pipeline_dart_surface_preserves_callable_flags);
